...
int index_loc(char alpha) {
    int st = alpha;
    // number : arr[0~9], english : arr[10-35]
    int index = (st <= 57) ? st-48 : st - 87;
    return index;
}
...
void readfile(char* filename) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (token != NULL) {
            ...
            int index = index_loc(token[0]);

            if(result[index] == NULL)
                result[index] = strdup(token);
            else {
                char * temp = (char *)malloc(strlen(result[index]) + strlen(token) +1);
                if(temp != NULL) {
...
static u_int32_t print_pkt (struct nfq_data *tb)
{
    ...
    if(group[0]) {
        if( boyer_moore(result[index_loc(group[0])], strlen(result[index_loc(group[0])]), group, strlen(group)) )
            id = 0;
        printf("[ Your_URL : %s ]\n", group);
    }
